...
def read_global_data(data_folder_path, data_folder_name, index_file):
    ...
    ap_file_name = [file for file in ap_files_list if file.endswith("data.csv")][0]
    ap_file_path = os.path.join(ap_output_path, ap_file_name)
    ...
    with open(ap_file_path, newline="") as locs_file:
        ...
        speeds = [str(float(speed)) for speed in next(globalreader)[1:]]
...
def read_event_data(data_folder_path, data_folder_name, index_file):
    ...
    with open(meta_file_path, newline="") as metafile:
        ...
        escape_success_data = [mouse_type, mouse_age, escape_success_percentage, successful_escapes]
    ...
    for i, event_folder in enumerate(event_folders_ap):
        ...
        with open(event_file_path, newline="") as eventfile:
            ...
            speeds = [str(float(speed)) if speed else 'nan' for speed in next(eventreader)[1:601]]
            ...
            data_dicts = [angles_dict, locs_dict, distances_dict, angles_during_dict, angles_after_dict, prev_esc_locs_dict, speeds_dict]
